...
Other notes:
a. If the IBERT GUI is used to re-customise the IP to change line rate, the ip/sources_1/ip/ibert_7series_gtx_0_ex/ibert_7series_gtx_0.xci
   file is updated with the changes. However, that doesn't change the line rate which gets used by the bitstream.
   To get the actual line rate updated need to:
   - Re-customise the IP with the line rate
   - Re-generate the example project, which updates the constraints in ibert_7series_gtx_0_ex/imports/example_ibert_7series_gtx_0.xdc
     which actuallty configure the line rate
   - Copy the updated constraints used to built by the ibert_7series_gtx_0_ex/ibert_7series_gtx_0_ex.xpr project created
     by create_example_project.tcl:
        cp ./ibert_7series_gtx_0_ex/imports/example_ibert_7series_gtx_0.xdc ./example_ibert_7series_gtx_0.xdc

   The need to re-generate the example project was found at 
   https://support.xilinx.com/s/question/0D52E00006hpm2oSAA/linerate-confusion-using-7series-vc707-ibert?language=en_US&t=1713718833059

b. If IBERT is set to generate a "fast clock" the links may be reported as up when the receiver isn't connected.
   See "63349 - My IBERT link is always up when Fast Clk is selected from RX side even when RX pins are unconnected"
   https://support.xilinx.com/s/article/63349
