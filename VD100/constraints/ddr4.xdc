# Constraints for the DDR4 memory using four Micron MT40A512M16LY-062E devices.
# This only defines package pins, since the constraints generated by the NoC IP contain:
# a. IO standards and related settings.
# b. The system clock frequency.
#
# @todo The VD100 schematic shows the LVDS output from the SiT9121AI-2B1-33E200.000000 oscillator
#       with a 3.3V supply is AC coupled to a Versal global clock buffer on bank 701 with VCCO 1.2V.
#       External bias resistors are fitted but the external termination resistor R16 is shown as
#       not fitted.
#       https://docs.amd.com/r/en-US/am010-versal-selectio/DQS_BIAS-DC_BIAS-and-AC-Coupling says
#         AC Coupling Recommendations
#         ~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
#         When receiving data from an AC coupled driver (like a clock source), care must be taken to
#         ensure the appropriate bias levels are selected so that the receiver's input threshold
#         requirements are met. If the receiver uses LVDS15 and resides in a 1.5V powered bank, 
#         a DC_BIAS value of DC_BIAS_1 along with DIFF_TERM_ADV setting of TERM_100 ensure that both
#         a DC bias level and termination are provided inside the IOB for an AC coupled input.
#         In scenarios where a 1.5V bank voltage is not used and AC coupling is required, it is
#         recommended that both an external bias and external termination are used:
#
#         - Do not use the optional internal differential termination.
#           - DIFF_TERM_ADV = TERM_NONE
#           - DIFF_TERM = FALSE (default)
#
#       The constraints from the NoC IP set the IO_STANDARD for the system clock as LVDS15.
#       Given the system clock is connected to a 1.2V bank are unable to set TERM_100 to compensate
#       for the lack of the external temination resistor R16. Not sure if this could cause reliability
#       issues with the DDR4.
...
set_property PACKAGE_PIN AB23 [get_ports {PL_CLK0_clk_p[0]}]
set_property PACKAGE_PIN AC23 [get_ports {PL_CLK0_clk_n[0]}]