...
static void display_usage (void)
{
    ...
    printf ("  buffers. Reducing increases the number of buffer used.\n");
...
static void parse_command_line_arguments (int argc, char *argv[])
{
    ...
    do
    {
        ...
        else if (opt_status >= 0)
        {
            ...
            {
                if ((sscanf (optarg, "%zu%c", &arg_max_buffer_size, &junk) != 1) ||
...
/**
 * @brief Perform a write/read test of DMA accessible memory using a pair of channels
                                                           ...
                                                           const uint32_t h2c_channel_id, const uint32_t c2h_channel_id)
{
    ...
    x2x_transfer_configuration_t h2c_transfer_configuration =
    {
    ...
    x2x_transfer_configuration_t c2h_transfer_configuration =
    {
        .dma_bridge_memory_size_bytes = design->dma_bridge_memory_size_bytes,
        .min_size_alignment = 1, /* The card memory is byte addressable */
        .num_descriptors = num_descriptors,
    ...
    uint32_t **const tx_buffers = calloc (num_descriptors, sizeof (uint32_t *));
    uint32_t **const rx_buffers = calloc (num_descriptors, sizeof (uint32_t *));

    printf ("\nTesting using %u buffers of size %zu bytes, H2C channel %u C2H channel %u\n",
    ...
    if (success)
    {
        ...
        for (size_t total_words = 0; success && (total_words < 0x100000000UL); total_words += ddr_size_words)
        {
            ...
            /* Verify the all card buffers have the expected contents */
...
int main (int argc, char *argv[])
{
    ...
    for (design_index = 0; design_index < designs.num_identified_designs; design_index++)
    {
        ...
        if (design->dma_bridge_present)
        {
            ...
            else if (design->dma_bridge_memory_size_bytes > 0)
            {
                     ...
                     h2c_channel_id++)
                {
                    for (c2h_channel_id = 0;
                         ...
                         c2h_channel_id++)
                    {
                        test_success = test_dma_accessible_memory_with_fixed_buffers
                                (design, vfio_device, h2c_channel_id, c2h_channel_id);
