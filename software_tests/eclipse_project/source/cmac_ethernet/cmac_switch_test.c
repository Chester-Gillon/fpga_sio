/*
 * @file cmac_switch_test.c
 * @date 29 Aug 2026
 * @author Chester Gillon
 * @details A variant of the https://github.com/chester-gillon/switch_test but which uses the CMAC to
 ...
 * The Working Counter has the least significant bits of the Address, used to validate received frames.
 *
 * The data[] field is variable length, and MAX_ETHERCAT_DATAGRAM_LEN is set to allow the maximum packet length
 * for CMAC_DRP_CTL_RX_MAX_PACKET_LEN_MASK (which is inclusive of the FCS).
 */
#define MAX_ETHERCAT_DATAGRAM_LEN 16347
typedef struct __attribute__((packed))
{
    ...
    uint8_t data[MAX_ETHERCAT_DATAGRAM_LEN]; /* Data to be read or written. Variable length. */
...
/* Command line arguments which specify the CMAC device and port used to send/receive test frames */
static char arg_cmac_device_location[PATH_MAX];
static uint32_t arg_cmac_tx_port_num;
static uint32_t arg_cmac_rx_port_num;
...
/* Optional command line argument to test loopback on the CMAC ports without needing a switch */
static bool arg_expect_cmac_loopback;


/* Optional command line argument to disable this program from collecting and reporting CMAC port statistics */
static bool arg_disable_port_statistics;


/* Optional command line arguments to set the min/max packet sizes tested, excluding the FCS.
 * Default value is for a MTU of 1500, which is formed from the EtherCAT Datagram header
...
static uint32_t arg_min_packet_len_excluding_fcs = 1518;
static uint32_t arg_max_packet_len_excluding_fcs = 1518;
...
typedef struct
{
    ...
    /* The CMAC design used to send/receive the test frames */
    fpga_design_t *cmac_design;
    /* Used to access the CMAC port registers.
     * Two sets for transmit and receive related registers, to allow for the command line arguments potentially specifying
     * different CMAC ports on the same design to be used for transmit and receive. */
    cmac_port_definition_t *cmac_tx_port;
    cmac_port_definition_t *cmac_rx_port;
    /* The number of CMAC ports for which statistics are collected */
    ...
    /* The CMAC port statistics over the duration of the test. Number of valid indices is num_ports_used_for_statistics.
     * When the transmit and receive ports are different both entries are used. Otherwise only the first entry is used. */
    cmac_port_statistics_t port_statistics[2];
    ...
    /* The index into the buffer of maximum length frames to be used for the next frame transmitted */
...
/* Per packet length statistics maintained for the entire test interval.
 * Added when used this program to investigate why a CMAC based design was intermittently having failures with some packet lengths.
 * The packet lengths are exclusive of the FCS. */
typedef struct
{
    /* The total number of transmit frames which used this packet length. */
    uint64_t num_tx_frames;
    /* The total mumber of missed frames for this packet length. */
    uint64_t num_missed_frames;
} per_packet_len_statistics_t;
static per_packet_len_statistics_t per_packet_len_statistics[sizeof (ethercat_frame_t) + 1];
...
/**
 * @brief Get the average total number of bit times one ethercat_frame_t occupies a network port for
 * @details
 *  Assumes the different packet lens are equally distributed between the network ports, which may not be true
 *  under some conditions of command line arguments for packet lengths and number of network ports.
 */
static uint32_t get_test_packet_bits (void)
{
    const uint32_t average_packet_len_bytes = (arg_min_packet_len_excluding_fcs + arg_max_packet_len_excluding_fcs) / 2u;
    const uint32_t test_packet_total_octets = (7 + /* Preamble */ \
                                  1 + /* Start frame delimiter */ \
                                  average_packet_len_bytes + \
                                  4 + /* Frame check sequence */ \
                                  12); /* Interpacket gap */
    const uint32_t bits_per_octet = 8;
    const uint32_t test_packet_bits = (test_packet_total_octets * bits_per_octet);

    return test_packet_bits;
}


/**
 * @brief Get the packet length, excluding the FCS, to be used for a sequence number
 * @param[in] sequence_number The sequence number to get the packet length for
 * @return The packet length in bytes.
 */
static size_t get_packet_len_for_sequence_number (const uint32_t sequence_number)
{
    const size_t num_different_packet_lens = (arg_max_packet_len_excluding_fcs - arg_min_packet_len_excluding_fcs) + 1;

    return arg_min_packet_len_excluding_fcs + (sequence_number % num_different_packet_lens);
}
...
{
    printf ("Usage %s: [-i <domain>:<bus>:<dev>.<func>] -n [cmac_port_num>|<cmac_tx_port_num>:<cmac_rx_port_num>] [-t <duration_secs>] [-d] [-p <port_list>] [-r <rate_mbps>] [-l] [-s] [-f <min_packet_len>:<max_packet_len>]\n", program_name);
    printf ("\n");
    ...
    printf ("  -n specifies which CMAC port to use. May be either:\n");
    printf ("     - A single number of the port to use for transmit and receive\n");
    printf ("     - A pair of colon delimited <cmac_tx_port_num>:<cmac_rx_port_num>\n");
    printf ("       to allow independent CMAC ports to be used for transmit and receive.\n");
    printf ("\n");
    ...
    printf ("  -l Expect the transmit frames to be looped back at the CMAC ports, rather\n");
    ...
    printf ("     This option allows testing of the CMAC without needing a switch.\n");
    printf ("  -s Disable collecting and reporting CMAC port statistics during the test.\n");
    printf ("     For when instead running cmac_statistics for a live update during the test.\n");
    printf ("  -f Specifies the range of packets lengths tested, excluding the FCS\n");
...
{
    bool cmac_port_num_specified = false;
    const char *const program_name = argv[0];
    const char *const optstring = "i:n:dt:p:r:lsf:";
    int option;
    char junk;
    uint32_t port_num;
    ...
    while (option != -1)
    {
        ...
        {
        case 'i':
            snprintf (arg_cmac_device_location, sizeof (arg_cmac_device_location), optarg);
            break;

        case 'n':
            if ((sscanf (optarg, "%" SCNu32 ":%" SCNu32 "%c", &tx_port_num, &rx_port_num, &junk) == 2) &&
                (tx_port_num < MAX_CMAC_PORTS_PER_DESIGN) && (rx_port_num < MAX_CMAC_PORTS_PER_DESIGN))
            {
                arg_cmac_tx_port_num = tx_port_num;
                arg_cmac_rx_port_num = rx_port_num;
            }
            else if ((sscanf (optarg, "%" SCNu32 "%c", &port_num, &junk) == 1) && (port_num < MAX_CMAC_PORTS_PER_DESIGN))
            {
                arg_cmac_tx_port_num = port_num;
                arg_cmac_rx_port_num = port_num;
            }
            else
            {
                printf ("Error: Invalid CMAC port(s) %s\n", optarg);
                exit (EXIT_FAILURE);
            }
            cmac_port_num_specified = true;
            break;
        ...
        case 'f':
            if ((sscanf (optarg, "%u:%u%c", &arg_min_packet_len_excluding_fcs, &arg_max_packet_len_excluding_fcs, &junk) != 2) ||
                (arg_min_packet_len_excluding_fcs > sizeof (ethercat_frame_t)) ||
                (arg_max_packet_len_excluding_fcs > sizeof (ethercat_frame_t)) ||
                (arg_min_packet_len_excluding_fcs > arg_max_packet_len_excluding_fcs))
            {
                printf ("Error: Invalid <min_packet_len>:<max_packet_len> %s\n", optarg);
                exit (EXIT_FAILURE);
            }
            break;
    ...
    if (!cmac_port_num_specified)
    {
        printf ("Error: The CMAC port(s) must be specified\n\n");
        display_usage (program_name);
    }
 ...
 * @details
 *   This is done since a VFIO open can reset the CMAC ports.
 *   Also has to ensure CMAC ports are enabled, since are disabled at reset.
 * @param[in/out] context The context to wait for the link to be ready.
 */
static void wait_receive_link_ready (frame_tx_rx_thread_context_t *const context)
{
    uint32_t rx_status;
    ...
    /* Ensure the ports are enabled for transmission and reception */
    cmac_enable_port (context->cmac_tx_port);
    cmac_enable_port (context->cmac_rx_port);

    /* Wait for the status to indicate the link is ready to receive */
    const uint32_t rx_link_ready_status_value = STAT_RX_STATUS_REG_STAT_RX_STATUS_MASK | STAT_RX_STATUS_REG_STAT_RX_ALIGNED_MASK;
    do
    {
        rx_status = read_reg32 (context->cmac_rx_port->cmac_control_status_statistics_regs, STAT_RX_STATUS_REG_OFFSET);
        rx_link_ready = rx_status == rx_link_ready_status_value;
    ...
    if (!first_wait)
    {
        /* @todo The 2 second delay is from using a 10G switch on other versions of this program.
         *       Haven't yet tested the CMAC with a switch. */
        printf ("Waiting 2 seconds after link came up for switch to accept packets (latched rx_status initial 0x%08X last 0x%08X)\n",
                first_rx_status, rx_status);
...
{
    const size_t packet_len = get_packet_len_for_sequence_number (sequence_number);
    ...
    frame->Length = (packet_len - offsetof (ethercat_frame_t, Cmd)) & ((1U << 11) - 1U);
    frame->Type = 1; /* EtherCAT commands */
    frame->Cmd = 11; /* Logical Memory Write */
    frame->Len = (packet_len - sizeof (frame->WKC) - offsetof (ethercat_frame_t, data)) & ((1U << 11) - 1U);

    /* The Length and Len fields are only 11 bits, which is insufficient for Jumbo frames. For populating the data contents
     * and WKC offset can't use frame->Len as wraps. */
    static uint8_t fill_value;
    const size_t wkc_offset = packet_len - sizeof (frame->WKC) - offsetof (ethercat_frame_t, data);
    for (uint32_t data_index = 0; data_index < wkc_offset; data_index++)
    {
    ...
    uint16_t *const WKC = (uint16_t *) &frame->data[wkc_offset];
    *WKC = frame->Address & UINT16_MAX;

}


/**
 * @brief Open the CMAC device used to send/receive test frames
 * @param[in/out] context The context being initialised.
 */
static void open_cmac_device (frame_tx_rx_thread_context_t *const context)
{
    /* Apply any device filter specified in the command line arguments */
    if (strlen (arg_cmac_device_location) > 0)
    {
        vfio_add_pci_device_location_filter (arg_cmac_device_location);
    }

    /* Open the FPGA designs which have an IOMMU group assigned */
    identify_pcie_fpga_designs (&context->designs);

    /* Obtain the CMAC design to be used for the test, and validate has the port number requested by the command line arguments */
    uint32_t num_designs_with_cmacs = 0;
    context->cmac_design = NULL;
    for (uint32_t design_index = 0; design_index < context->designs.num_identified_designs; design_index++)
    {
        ...
        if (candidate_design->num_cmac_ports > 0)
        {
            num_designs_with_cmacs++;
            if (context->cmac_design == NULL)
            {
                context->cmac_design = candidate_design;
            }
        }
    }

    if (num_designs_with_cmacs == 0)
    {
        printf ("Error: Found no design with a supported CMAC\n");
        exit (EXIT_FAILURE);
    }
    else if (num_designs_with_cmacs > 1)
    {
        printf ("Error: Found %u designs with CMAC. Use -i option to select a single design\n", num_designs_with_cmacs);
        exit (EXIT_FAILURE);
    }
    else if (!context->cmac_design->cmac_ports[arg_cmac_tx_port_num].configured_features[CMAC_FEATURE_PACKET_TX])
    {
        printf ("Error: Design %s doesn't have requested CMAC Tx port %u\n",
                fpga_design_names[context->cmac_design->design_id], arg_cmac_tx_port_num);
        exit (EXIT_FAILURE);
    }
    else if (!context->cmac_design->cmac_ports[arg_cmac_rx_port_num].configured_features[CMAC_FEATURE_PACKET_RX])
    {
        printf ("Error: Design %s doesn't have requested CMAC Rx port %u\n",
                fpga_design_names[context->cmac_design->design_id], arg_cmac_rx_port_num);
        exit (EXIT_FAILURE);
    }

    context->cmac_tx_port = &context->cmac_design->cmac_ports[arg_cmac_tx_port_num];
    context->cmac_rx_port = &context->cmac_design->cmac_ports[arg_cmac_rx_port_num];
    context->xdma_overall_success = true;

    uint32_t rx_min_packet_len_including_fcs;
    uint32_t rx_max_packet_len_including_fcs;
    cmac_get_rx_min_max_packet_lens (context->cmac_rx_port, &rx_min_packet_len_including_fcs, &rx_max_packet_len_including_fcs);
    const uint32_t rx_min_packet_len_excluding_fcs = (uint32_t) (rx_min_packet_len_including_fcs - sizeof (uint32_t));
    const uint32_t rx_max_packet_len_excluding_fcs = (uint32_t) (rx_max_packet_len_including_fcs - sizeof (uint32_t));
    if ((arg_min_packet_len_excluding_fcs < rx_min_packet_len_excluding_fcs) ||
        (arg_max_packet_len_excluding_fcs > rx_max_packet_len_excluding_fcs))
    {
        printf ("Requested packet lengths %u:%u, excluding of FCS, are outside the receive port configured range %u:%u\n",
                arg_min_packet_len_excluding_fcs, arg_max_packet_len_excluding_fcs,
                rx_min_packet_len_excluding_fcs, rx_max_packet_len_excluding_fcs);
        exit (EXIT_FAILURE);
    }

    if (arg_disable_port_statistics)
    {
        /* Command line has disabled CMAC statistics collection */
        context->num_ports_used_for_statistics = 0;
    }
    else if (arg_cmac_tx_port_num == arg_cmac_rx_port_num)
    {
        /* Collect statistics from the same port used for transmit and receive */
        context->statistics_port_nums[0] = arg_cmac_tx_port_num;
    ...
    else
    {
        ...
        context->statistics_port_nums[0] = arg_cmac_tx_port_num;
        context->statistics_port_nums[1] = arg_cmac_rx_port_num;
    ...
    /* Configure XDMA transmit to use a variable size buffers, with each buffer up to the maximum test frame size.
     * Each transmit buffer is cache line aligned. */
    const x2x_transfer_configuration_t h2c_transfer_configuration =
    {
        .dma_bridge_memory_size_bytes = context->cmac_design->dma_bridge_memory_size_bytes,
        .dma_bridge_memory_base_address = context->cmac_design->dma_bridge_memory_base_address,
        ...
        .channel_id = arg_cmac_tx_port_num,
        .bytes_per_buffer = 0, /* Offset and size set at runtime */
        ...
        .vfio_device = context->cmac_design->vfio_device,
        .bar_index = context->cmac_design->dma_bridge_bar,
    ...
    const x2x_transfer_configuration_t c2h_transfer_configuration =
    {
        ...
        .channel_id = arg_cmac_rx_port_num,
    ...
    /* If the design contains an AXI stream switch between the XDMA and CMAC data ports, set the default routing if no specific
     * routing has already been configured. That is allow the XMDA to be connected to the CMAC data ports, to allow for actual
     * packet transmission and reception by this program. */
    if (context->cmac_design->axi_switch_regs != NULL)
    {
        configure_routing_for_device (context->cmac_design, &context->routing);
    }
    ...
    allocate_vfio_dma_mapping (context->cmac_design->vfio_device, &context->descriptors_mapping, descriptors_allocation_size,
    ...
    allocate_vfio_dma_mapping (context->cmac_design->vfio_device,
            &context->h2c_data_mapping,
            h2c_transfer_configuration.num_descriptors * vfio_align_cache_line_size (sizeof (ethercat_frame_t)),
    ...
    if (context->xdma_overall_success)
    {
         ...
         * This program doesn't attempt to flush any frames which might be in the internal AXI steam FIFOs between the CMAC
...
/**
 * @brief Close the CMAC device used to send/receive test frames, freeing the resources.
 * @param[in/out] context The context being closed.
 */
static void close_cmac_device (frame_tx_rx_thread_context_t *const context)
{
...
/**
 * @brief Collect the CMAC port statistics
 * @param[in/out] context Context to get the CMAC port statistics for
 */
static void collect_port_statistics (frame_tx_rx_thread_context_t *const context)
{
    ...
    {
        cmac_snapshot_port_statistics (context->cmac_design, context->statistics_port_nums[port_index],
                &context->port_statistics[port_index]);
    }

    for (port_index = 0; port_index < context->num_ports_used_for_statistics; port_index++)
    {
        cmac_read_port_statistics (&context->port_statistics[port_index]);
    }
}


/**
 * @brief Display the CMAC port statistics over the duration of the test
 * @details This function has copied code from cmac_display_port_statistics(), since needs to use console_printf() so that the
 *          output gets saved to the log file.
 * @param[in] context Context containing the CMAC port statistics
 */
static void display_port_statistics (const frame_tx_rx_thread_context_t *const context)
{
    ...
    {
        const cmac_port_statistics_t *const stats = &context->port_statistics[port_index];
                            ...
                            frame_record_t *const frame_record)
{
    ...
    else
    {
        ...
        frame_record->len = get_packet_len_for_sequence_number (frame->Address);
    }
    ...
    bool is_test_frame =
            (frame_record->len >= arg_min_packet_len_excluding_fcs) && (frame_record->len <= arg_max_packet_len_excluding_fcs);
    ...
    if (is_test_frame)
    {
        const size_t expected_packet_len = get_packet_len_for_sequence_number (frame->Address);
        const uint16_t expected_wkc = frame->Address & UINT16_MAX;
        const size_t wkc_offset = expected_packet_len - sizeof (frame->WKC) - offsetof (ethercat_frame_t, data);
        uint16_t *const WKC = (uint16_t *) &frame->data[wkc_offset];

        is_test_frame = (frame_record->len == expected_packet_len) && (*WKC == expected_wkc);
    }
...
static void handle_pending_rx_frame (frame_tx_rx_thread_context_t *const context, frame_record_t *const frame_record)
{
    ...
    const uint16_t expected_vlan = arg_expect_cmac_loopback ?
            /* When testing CMAC loopback expect to receive on the source VLAN */
    ...
    if (frame_record->vlan_id == expected_vlan)
    {
        ...
        while ((!pending_match_found) && (pending->num_pending_rx_frames > 0))
        {
            ...
            else
            {
                ...
                const size_t missed_packet_len =
                        get_packet_len_for_sequence_number (pending->pending_rx_sequence_numbers[pending->rx_index]);
                per_packet_len_statistics[missed_packet_len].num_missed_frames++;
            }
...
static void transmit_next_test_frame (frame_tx_rx_thread_context_t *const context)
{
    ...
    const size_t packet_len = get_packet_len_for_sequence_number (context->next_tx_sequence_number);
    ethercat_frame_t *const tx_frame = x2x_populate_stream_transfer (&context->h2c_transfer, packet_len,
            context->next_tx_buffer_index * vfio_align_cache_line_size (sizeof (ethercat_frame_t)));
    ...
    per_packet_len_statistics[packet_len].num_tx_frames++;
...
/**
 * @brief Write a CSV file which contains the per packet length statistics for the entire test
 * @param[in] per_packet_len_csv_filename Name of the CSV file to create
 */
static void write_per_packet_len_csv_file (const char *const per_packet_len_csv_filename)
{

    uint32_t packet_len;
    /* Create CSV file and write headers */
    FILE *const csv_file = fopen (per_packet_len_csv_filename, "w");
    if (csv_file == NULL)
    {
        console_printf ("Failed to create %s\n", per_packet_len_csv_filename);
        exit (EXIT_FAILURE);
    }
    fprintf (csv_file, "packet len (bytes excluding FCS),Num tx frames,Num missed frames\n");

    /* Write one row per packet length used during the test */
    for (packet_len = 0; packet_len < VFIO_NELEMENTS (per_packet_len_statistics); packet_len++)
    {
        const per_packet_len_statistics_t *const statistics = &per_packet_len_statistics[packet_len];

        if (statistics->num_tx_frames > 0)
        {
            fprintf (csv_file, "%u,%" PRIu64 ",%" PRIu64 "\n", packet_len, statistics->num_tx_frames, statistics->num_missed_frames);
        }
    }
                                         ...
                                         const frame_test_statistics_t *const statistics)
{
    ...
    const double per_port_mbps = ((frame_rate * (double) get_test_packet_bits ()) / (double) num_tested_port_indices) / 1E6;
...
int main (int argc, char *argv[])
{
    ...
    /* Check that ethercat_frame_t has the expected size. This is 14 bits, those used in CMAC_DRP_CTL_RX_MAX_PACKET_LEN_MASK,
     * minus 4 bytes for the FCS which is inclusive in CMAC_DRP_CTL_RX_MAX_PACKET_LEN_MASK. */
    check_assert (sizeof (ethercat_frame_t) == 16379,
            "sizeof (ethercat_frame_t) unexpected value of %" PRIuPTR, sizeof (ethercat_frame_t));

    /* Read the commandline arguments */
    read_command_line_arguments (argc, argv);

    /* Open the CMAC device specified on the command line, which validates the device is present and supports
    ...
    open_cmac_device (tx_rx_thread_context);
    ...
    char per_packet_len_csv_filename[160];
    ...
    snprintf (per_packet_len_csv_filename, sizeof (per_packet_len_csv_filename), "%s_per_packet_len_%s.csv", date_time_str, OS_NAME);
    ...
    /* The bit rate of the CMAC port is fixed at 100Gbps */
    const int64_t injection_port_bit_rate_mbps = 100000L;
    const int64_t injection_port_bit_rate = 1000000L * injection_port_bit_rate_mbps;
    wait_receive_link_ready (tx_rx_thread_context);
    ...
    {
        const double limited_frame_rate = (double) requested_switch_under_test_bit_rate / (double) get_test_packet_bits ();
    ...
    console_printf ("Using design %s device %s Tx port %u Rx port %u\n",
            fpga_design_names[tx_rx_thread_context->cmac_design->design_id],
            tx_rx_thread_context->cmac_design->vfio_device->device_name, arg_cmac_tx_port_num, arg_cmac_rx_port_num);
    ...
    console_printf ("Expect CMAC loopback = %s\n", arg_expect_cmac_loopback ? "Yes" : "No");
    console_printf ("Disable CMAC port statistics = %s\n", arg_disable_port_statistics ? "Yes" : "No");
    console_printf ("Min packet len (excluding FCS) = %u\n", arg_min_packet_len_excluding_fcs);
    console_printf ("Max packet len (excluding FCS) = %u\n", arg_max_packet_len_excluding_fcs);
    ...
    /* If testing with varying packet lengths, write the per packet length statistics */
    if (arg_min_packet_len_excluding_fcs != arg_max_packet_len_excluding_fcs)
    {
        write_per_packet_len_csv_file (per_packet_len_csv_filename);
    }
