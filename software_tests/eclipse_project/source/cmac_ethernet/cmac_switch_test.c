...
{
    printf ("Usage %s: [-i <domain>:<bus>:<dev>.<func>] -n [cmac_port_num>|<cmac_tx_port_num>:<cmac_rx_port_num>] [-t <duration_secs>] [-d] [-p <port_list>] [-r <rate_mbps>] [-l] [-s]\n", program_name);
    printf ("\n");
...
static void read_command_line_arguments (const int argc, char *argv[])
{
    ...
    const char *const optstring = "i:n:dt:p:r:ls";
    int option;
    char junk;
    uint32_t port_num;
...
static void open_cmac_device (frame_tx_rx_thread_context_t *const context)
{
    ...
    /* Configure XDMA transmit to use a queue of fixed size buffers, based upon the size of the test frame.
     * Since the size of the test frame isn't aligned to cache line boundary, this means the transmit buffers aren't
     * cache line aligned. */
    const x2x_transfer_configuration_t h2c_transfer_configuration =
    {
        ...
        .bytes_per_buffer = sizeof (ethercat_frame_t),
