...
#define IIC_ISR_ARBITRATION_LOST_MASK                       (1 << 0)
#define IIC_ISR_TRANSMIT_ERROR_SLAVE_TRANSMIT_COMPLETE_MASK (1 << 1)
#define IIC_ISR_TRANSMIT_FIFO_EMPTY_MASK                    (1 << 2)
#define IIC_ISR_RECEIVE_FIFO_FULL_MASK                      (1 << 3)
#define IIC_ISR_IIC_BUS_IS_NOT_BUSY_MASK                    (1 << 4)
#define IIC_ISR_ADDRESSED_AS_SLAVE_MASK                     (1 << 5)
#define IIC_ISR_NOT_ADDRESSED_AS_SLAVE_MASK                 (1 << 6)
...
#define IIC_CR_EN_MASK (1 << 0) /* AXI IIC Enable. This bit must be set before any other CR bits have any effect.
                                   0 = resets and disables the AXI IIC controller but not the registers or FIFOs
                                   1 = enables the AXI IIC controller */
#define IIC_CR_TX_FIFO_RESET_MASK (1 << 1) /* Transmit FIFO Reset. This bit must be set to flush the FIFO if either
                                              (a) arbitration is lost or (b) if a transmit error occurs.
                                              0 = transmit FIFO normal operation
                                              1 = resets the transmit FIFO */
#define IIC_CR_MSMS_MASK (1 << 2) /* Master/Slave Mode Select. When this bit is changed from 0 to 1, the
...
#define IIC_CR_TX_MASK (1 << 3) /* Transmit/Receive Mode Select. This bit selects the direction of master/ slave transfers.
...
#define IIC_CR_TXAK_MASK (1 << 4) /* Transmit Acknowledge Enable. This bit specifies the value driven onto
...
#define IIC_CR_RSTA_MASK (1 << 5) /* Repeated Start. Writing a 1 to this bit generates a repeated START
...
#define IIC_CR_GC_EN_MASK (1 << 6) /* General Call Enable. Setting this bit High allows the AXI IIC to respond
...
#define IIC_SR_ABGC_MASK (1 << 0) /* Addressed By a General Call. This bit is set to 1 when another
                                     master has issued a general call and the general call enable bit is set to 1, CR(6) = 1. */
#define IIC_SR_AAS_MASK (1 << 1) /* Addressed as Slave. When the address on the IIC bus matches the
...
#define IIC_SR_BB_MASK (1 << 2) /* Bus Busy. This bit indicates the status of the IIC bus. This bit is set
...
#define IIC_SR_SRW_MASK (1 << 3) /* Slave Read/Write. When the IIC bus interface has been
...
#define IIC_SR_TX_FIFO_FULL_MASK (1 << 4) /* Transmit FIFO full. This bit is set High when the transmit FIFO is full. */
#define IIC_SR_RX_FIFO_FULL_MASK (1 << 5) /* Receive FIFO full. This bit is set High when the receive FIFO is full.
...
#define IIC_SR_RX_FIFO_EMPTY_MASK (1 << 6) /* Receive FIFO empty. This is set High when the receive FIFO is empty. */
#define IIC_SR_TX_FIFO_EMPTY_MASK (1 << 7) /* Transmit FIFO empty. This bit is set High when the transmit FIFO is empty.
...
#define IIC_TX_FIFO_START_MASK (1 << 8) /* Start. The dynamic start bit can be used to send a start or
...
#define IIC_TX_FIFO_STOP_MASK (1 << 9) /* Stop. The dynamic stop bit can be used to send an IIC stop
